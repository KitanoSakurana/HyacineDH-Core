...
public class AddMazeBuff : TaskConfigInfo
{
    ...
    public new static TaskConfigInfo LoadFromJsonObject(JObject obj)
    {
        ...
        if (obj.TryGetValue(nameof(TargetType), out var value))
        {
            ...
            var classType =
                System.Type.GetType(
                    $"HyacineCore.Server.Data.Config.Task.{targetType?["Type"]?.ToString().Replace("RPG.GameCore.", "")}");
            classType ??= System.Type.GetType("HyacineCore.Server.Data.Config.Task.TargetEvaluator");
        ...
        if (obj.TryGetValue(nameof(Condition), out value))
        {
            ...
            var classType =
                System.Type.GetType(
                    $"HyacineCore.Server.Data.Config.Task.{condition?["Type"]?.ToString().Replace("RPG.GameCore.", "")}");
            classType ??= System.Type.GetType("HyacineCore.Server.Data.Config.Task.PredicateConfigInfo");
        ...
        if (obj.TryGetValue(nameof(BindingAffectedTarget), out value))
        {
            ...
            var classType =
                System.Type.GetType(
                    $"HyacineCore.Server.Data.Config.Task.{bindingAffectedTarget?["Type"]?.ToString().Replace("RPG.GameCore.", "")}");
            classType ??= System.Type.GetType("HyacineCore.Server.Data.Config.Task.TargetEvaluator");
