        ...
        try
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                if (bytesReceived != 0 && bytesReceived <= _mtu)
                {
                    ...
                    try
                    {
                        if (ShouldShowHandshakeLog() && bytesReceived <= 96)
                        {
                            ...
                            if (count <= 10)
                            {
                                ...
                                var headHex = Convert.ToHexString(result.Buffer.AsSpan(0, headLen));
                                Logger.Debug($"UDP recv: remote={result.RemoteEndPoint} bytes={bytesReceived} head16={headHex}");
                            }
                        }
                    }
                    catch
                    ...
                    // Handshake packets are sent over UDP (outside KCP stream).
                    // Some clients may send extra bytes, so don't require exact length == 20.
                    if (bytesReceived >= HyacineCoreConnection.HANDSHAKE_SIZE)
                    {
                        var code = (result.Buffer[0] << 24) | (result.Buffer[1] << 16) | (result.Buffer[2] << 8) |
                                   result.Buffer[3];

                        // 0x000000FF / 0x00000194 / -934149376 (0xC85B0D80)
                        if (code == 0x000000FF || code == 0x00000194 || code == unchecked((int)0xC85B0D80))
                        {
                            await HyacineCoreListener.HandleHandshake(result);
                            continue;
                        }
                    }
