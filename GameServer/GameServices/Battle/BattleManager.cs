...
public class BattleManager(PlayerInstance player) : BasePlayerManager(player)
{
        ...
        string? mazeAbilityStr = null)
    {
        ...
        if (targetList.Count > 0)
        {
            ...
            // Special handling (LC compatibility): Castorice (瑕蝶/遐蝶) maze skill needs extra battle buffs
            // or the client may keep the maze-skill scene state when entering battle.
            if (castAvatar != null)
            {
                var casterId = castAvatar.AvatarInfo.BaseAvatarId;
                if (casterId == 1407 && !string.IsNullOrEmpty(mazeAbilityStr) &&
                    (mazeAbilityStr == "LocalPlayer_Castorice_MazeSkill_Input" ||
                     mazeAbilityStr.Contains("Castorice_MazeSkill", StringComparison.OrdinalIgnoreCase)))
                {
                    foreach (var buffId in new[] { 140701, 140702, 140704 })
                        battleInstance.Buffs.Add(new MazeBuff(buffId, 1, -1) { OwnerAvatarId = casterId });
                }
            }
    ...
    public async ValueTask EndBattle(PVEBattleResultCsReq req)
    {
        ...
        var removedSceneMonsters = new List<EntityMonster>();
        switch (req.EndStatus)
        {
            case BattleEndStatus.BattleEndWin:
                ...
                {
                    dropItems.AddRange(await monster.Kill(false));
                    removedSceneMonsters.Add(monster);
                }
                // Spend stamina
        ...
        // Some clients ignore scene refresh packets while in battle; resend the removals after battle result.
        if (removedSceneMonsters.Count > 0 && Player.SceneInstance != null)
            await Player.SendPacket(new PacketSceneGroupRefreshScNotify(Player, removeEntity: removedSceneMonsters.Cast<BaseGameEntity>().ToList()));
    }
}
