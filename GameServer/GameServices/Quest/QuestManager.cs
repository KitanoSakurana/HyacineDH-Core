...
public class QuestManager(PlayerInstance player) : BasePlayerManager(player)
{
    ...
    public async ValueTask<(Retcode, List<ItemData>?)> TakeQuestReward(int questId)
    {
        ...
        // When quests are disabled, the client may still attempt to claim quest-bound rewards.
        // Treat them as completed, but still enforce "only once" via stored quest status.
        if (!ConfigManager.Config.ServerOption.EnableQuest)
        {
            if (QuestData.Quests.TryGetValue(questId, out var existing) && existing.QuestStatus == QuestStatus.QuestClose)
                return (Retcode.RetQuestRewardAlreadyTaken, null);

            // Create/update a record so the reward cannot be claimed repeatedly.
            if (!QuestData.Quests.TryGetValue(questId, out var questRecord))
            {
                questRecord = new QuestInfo
                {
                    QuestId = questId
                };
                QuestData.Quests[questId] = questRecord;
            }

            // Mark as closed after taking the reward.
            questRecord.QuestStatus = QuestStatus.QuestClose;
            questRecord.FinishTime = DateTime.Now.ToUnixSec();
            questRecord.Progress = 0;

            var rewardItems = await Player.InventoryManager!.HandleReward(questExcel.RewardID);
            await Player.SendPacket(new PacketPlayerSyncScNotify(questRecord));
            return (Retcode.RetSucc, rewardItems);
        }
    ...
    {
        // When quests are disabled, treat untracked quests as finished (so they appear unlocked),
        // but preserve the stored status for already-claimed rewards (QuestClose etc).
        if (!ConfigManager.Config.ServerOption.EnableQuest)
            return QuestData.Quests.TryGetValue(questId, out var info) ? info.QuestStatus : QuestStatus.QuestFinish;
